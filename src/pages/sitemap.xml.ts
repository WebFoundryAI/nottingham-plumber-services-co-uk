...
function generateSitemapUrls(): SitemapUrl[] {
  ...
  for (const [i, location] of INDEXED_LOCATIONS.entries()) {
    // Main location page (Nottingham gets higher priority)
    const isNottingham = location.slug === 'nottingham';
    urls.push({
      loc: `/locations/${location.slug}/`,
      priority: isNottingham ? 0.9 : 0.8,
      changefreq: 'weekly',
      lastmod: getStaggeredDate(isNottingham ? 1 : 2 + i),
    });
